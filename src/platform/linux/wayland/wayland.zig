...
pub fn main() !void {
    ...
    const shared_memory_pool_fd = try std.posix.memfd_create("my-wayland-framebuffer", 0);
    try std.posix.ftruncate(shared_memory_pool_fd, shared_memory_pool_len);
    const shared_memory_pool_bytes = try std.posix.mmap(null, shared_memory_pool_len, std.posix.PROT.READ | std.posix.PROT.WRITE, .{ .TYPE = .SHARED }, shared_memory_pool_fd, 0);
...
pub fn writeWlShmRequestCreatePool(socket: std.net.Stream, wl_shm_id: u32, next_id: *u32, fd: std.posix.fd_t, fd_len: i32) !u32 {
